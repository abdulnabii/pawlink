...
interface RecoveryMapProps {
  lastSeenLat?: number | string | null;
  lastSeenLng?: number | string | null;
...
export function RecoveryMap({
    ...
    import("leaflet")
      .then((L) => {
        ...
        try {
          ...
          const safeLastSeenLat =
            lastSeenLat !== null && lastSeenLat !== undefined
              ? parseFloat(String(lastSeenLat))
              : NaN;
          const safeLastSeenLng =
            lastSeenLng !== null && lastSeenLng !== undefined
              ? parseFloat(String(lastSeenLng))
              : NaN;
          const hasLastSeen = !isNaN(safeLastSeenLat) && !isNaN(safeLastSeenLng);

          const validLocationEvents = (Array.isArray(locationEvents) ? locationEvents : [])
            .map((loc) => {
          ...
          const primaryLat =
            validLocationEvents[0]?.lat ||
            (hasLastSeen ? safeLastSeenLat : 24.8138);
          const primaryLng =
            validLocationEvents[0]?.lng ||
            (hasLastSeen ? safeLastSeenLng : 67.0299);
          ...
          if (hasLastSeen) {
            const lastSeenMarker = L.marker([safeLastSeenLat, safeLastSeenLng], {
              icon: L.divIcon({
            ...
            `);
            bounds.extend([safeLastSeenLat, safeLastSeenLng]);
          }

          // 2. Plot Finder Location Events
          validLocationEvents.forEach((loc, idx) => {
            const isLatest = idx === 0;

            const finderMarker = L.marker([loc.lat, loc.lng], {
              icon: L.divIcon({
                className: "custom-div-icon",
                html: `
                  <div style="background-color: ${isLatest ? "#0d9488" : "#64748b"}; width: ${
                  isLatest ? "36px" : "28px"
                }; height: ${
                  isLatest ? "36px" : "28px"
                }; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; border: 3px solid white; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.3);">
                    🐾
                  </div>
                `,
            ...
            // Accuracy Circle
            if (loc.accuracy && typeof loc.accuracy === "number") {
              try {
            ...
            const timeStr = loc.createdAt
              ? new Date(loc.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })
              : "";
            ...
            finderMarker.bindPopup(`
              <div style="font-family: sans-serif; font-size: 13px;">
                ...
                ${timeStr ? `<small style="color: #64748b;">${timeStr}</small>` : ""}
          ...
          console.warn("RecoveryMap setup notice:", err);
        }
      })
      .catch((err) => {
        console.warn("Leaflet import notice:", err);
      });
  ...
  const parsedLastSeenLat =
    lastSeenLat !== null && lastSeenLat !== undefined
      ? parseFloat(String(lastSeenLat))
      : NaN;
  const parsedLastSeenLng =
    lastSeenLng !== null && lastSeenLng !== undefined
      ? parseFloat(String(lastSeenLng))
      : NaN;
  const hasCoordinates =
    (!isNaN(parsedLastSeenLat) && !isNaN(parsedLastSeenLng)) ||
    (Array.isArray(locationEvents) && locationEvents.length > 0);
  ...
  return (
    <div className="relative w-full h-80 rounded-2xl overflow-hidden border border-slate-200 shadow-inner bg-slate-100">
      ...
      <div className="absolute top-3 left-3 z-[400] bg-white/95 backdrop-blur-sm px-3.5 py-2 rounded-xl shadow-md border border-slate-200 text-xs font-semibold text-slate-800 flex items-center gap-2">
        ...
        <span>
          {Array.isArray(locationEvents) && locationEvents.length > 0
            ? `${locationEvents.length} Finder Location Point${
                locationEvents.length > 1 ? "s" : ""
              }`
            : lastSeenLocation
        ...
        <div className="absolute inset-0 bg-slate-900/10 backdrop-blur-[1px] flex items-center justify-center p-6 text-center z-[400]">
          <div className="bg-white/95 p-4 rounded-xl shadow-lg border border-slate-200 max-w-xs">
            ...
            <p className="text-xs text-slate-500 mt-1">
              When a finder shares their location or you set a last known area,
              it will appear here on this map.
            </p>
          </div>
        </div>
      )}
