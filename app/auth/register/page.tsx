...
function RegisterForm() {
  ...
  const rawPlan = (searchParams.get("plan") || "FREE").toUpperCase();
  const plan = ["FREE", "PLUS", "PRO"].includes(rawPlan) ? rawPlan : "FREE";
  ...
  return (
    <div className="bg-slate-800/80 backdrop-blur-md py-8 px-6 shadow-2xl rounded-3xl sm:px-10 border border-slate-700">
      ...
      <div className="mb-6 p-3 rounded-2xl bg-slate-900 border border-slate-700 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div className="text-left">
            <p className="text-[10px] uppercase font-bold text-slate-400">Selected Plan</p>
            <p className="text-xs font-black text-white">
              {plan === "PRO" ? "👑 Pro Household (Rs 2,999/mo)" : plan === "PLUS" ? "⚡ Plus Recovery (Rs 1,499/mo)" : "🛡️ Basic ID (Free Forever)"}
            </p>
          </div>
        </div>
        <Link href="/#pricing" className="text-[10px] text-teal-400 hover:underline font-bold">
          Change
        </Link>
      </div>
